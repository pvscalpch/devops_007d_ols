...
Este pipeline evita que llegue a la rama estable código que no compila o cuyos tests fallen. El secreto `JWT_SECRET` se inyecta desde GitHub Secrets y no se escribe en el código porque el repositorio es público y esa clave es la firma de los tokens: cualquiera que la lea puede fabricar un token y hacerse pasar por cualquier usuario del sistema. Si el pipeline fallara en un Pull Request hacia `main`, no se debería poder mergear, porque `main` es la rama estable o funcional; para eso se configura una regla de protección de rama que exige que el pipeline funcione antes de poder hacer el merge. Para convertir esta integración continua en un despliegue continuo faltaría un paso que tome el artefacto generado y lo despliegue en un entorno real, como un Docker, un servidor o un servicio cloud.
...
Las herramientas de IA que utilizamos fueron Claude, más que nada para corregir errores al momento de hacer los Pull Requests que se fueron para una rama que no era, y en la redacción del README.
...
Por mi parte, las cosas que aprendí al hacer esta primera evaluación fue a trabajar de una forma distinta a como lo había hecho antes, debido a que en su momento trabajé con GitHub Flow, donde cada uno tenía una rama temporal feature en la que hacía sus avances; clonaba la rama de mi compañero y le añadía las cosas que faltaban. Esto fue un desafío para mí, debido a que me costaba entenderlo desde un inicio. Las cosas que contribuí fueron arreglar el error del `mvnw` que fallaba solo en Linux, corregir el error de que los Pull Requests se iban a la rama equivocada y agregar el endpoint `/api/health` con su regla de seguridad.
